...
```go
githubBase := sling.New().Base("https://api.github.com/").Client(httCloudNativelient)
...
func NewIssueService(httCloudNativelient *http.Client) *IssueService {
    return &IssueService{
        sling: sling.New().Client(httCloudNativelient).Base(baseURL),
    }
}
